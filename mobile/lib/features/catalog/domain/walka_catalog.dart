...
class WalkaStorefrontConfig {
  const WalkaStorefrontConfig({required this.brand, required this.release, required this.apiVersion, required this.purchaseMode});
  final String brand;
  final String release;
  final String apiVersion;
  final String purchaseMode;
  factory WalkaStorefrontConfig.fromJson(Map<String, dynamic> json) => WalkaStorefrontConfig(
  ...
  Map<String, dynamic> toJson() => <String, dynamic>{'brand': brand, 'release': release, 'api_version': apiVersion, 'purchase_mode': purchaseMode};
}
...
class WalkaCatalogProduct {
  const WalkaCatalogProduct({
    required this.id, required this.name, required this.category, required this.features, required this.facts, required this.variants,
    this.shortDescription, this.highlights = const <String>[], this.featured = false, this.presentationOrder = 0,
  });
  final String id;
  final String name;
  ...
  factory WalkaCatalogProduct.fromJson(Map<String, dynamic> json) {
    ...
    if (rawFeatured != null && rawFeatured is! bool) throw const FormatException('featured must be a boolean when present.');
    final Object? rawOrder = json['presentation_order'];
    if (rawOrder != null && (rawOrder is! int || rawOrder < 0)) throw const FormatException('presentation_order must be a non-negative integer.');
    final List<String> parsedHighlights = (rawHighlights as List<dynamic>? ?? const <dynamic>[]).map((dynamic value) {
      if (value is! String || value.trim().isEmpty) throw const FormatException('Product highlights must be strings.');
      return value;
    }).toList(growable: false);
    return WalkaCatalogProduct(
      id: _requiredString(json, 'id'), name: _requiredString(json, 'name'), category: _requiredString(json, 'category'),
      ...
      variants: rawVariants.map((dynamic value) {
        if (value is! Map) throw const FormatException('Product variants must be objects.');
        return WalkaCatalogVariant.fromJson(Map<String, dynamic>.from(value));
      }).toList(growable: false),
    );
  }
  Map<String, dynamic> toJson() => <String, dynamic>{
        'id': id, 'name': name, 'category': category, 'features': features, 'short_description': shortDescription,
        'highlights': highlights, 'featured': featured, 'presentation_order': presentationOrder, 'facts': facts,
        'variants': variants.map((WalkaCatalogVariant variant) => variant.toJson()).toList(growable: false),
      };
}

class WalkaCatalogPayload {
  const WalkaCatalogPayload({required this.products, required this.release, required this.apiVersion, required this.purchaseMode});
  ...
  factory WalkaCatalogPayload.fromJson(Map<String, dynamic> json) {
    ...
    return WalkaCatalogPayload(
      products: rawProducts.map((dynamic value) {
        if (value is! Map) throw const FormatException('Catalog products must be objects.');
        return WalkaCatalogProduct.fromJson(Map<String, dynamic>.from(value));
      }).toList(growable: false),
      release: _requiredString(meta, 'release'), apiVersion: _requiredString(meta, 'api_version'), purchaseMode: _requiredString(meta, 'purchase_mode'),
    );
  }
}

class WalkaCatalogSnapshot {
  const WalkaCatalogSnapshot({required this.config, required this.products, required this.source, required this.fetchedAt});
  final WalkaStorefrontConfig config;
  final List<WalkaCatalogProduct> products;
  final WalkaCatalogSource source;
  final DateTime fetchedAt;
  bool get isStale => source != WalkaCatalogSource.remote;
  Iterable<WalkaCatalogVariant> get variants sync* { for (final WalkaCatalogProduct product in products) { yield* product.variants; } }
  WalkaCatalogProduct? productById(String id) { for (final WalkaCatalogProduct product in products) { if (product.id == id) return product; } return null; }
  WalkaCatalogVariant? variantById(String id) { for (final WalkaCatalogVariant variant in variants) { if (variant.id == id) return variant; } return null; }
  WalkaCatalogSnapshot asSource(WalkaCatalogSource nextSource) => WalkaCatalogSnapshot(config: config, products: products, source: nextSource, fetchedAt: fetchedAt);
  Map<String, dynamic> toJson() => <String, dynamic>{
        'config': config.toJson(), 'products': products.map((WalkaCatalogProduct product) => product.toJson()).toList(growable: false),
        'fetched_at': fetchedAt.toUtc().toIso8601String(),
      };
  factory WalkaCatalogSnapshot.fromJson(Map<String, dynamic> json, {WalkaCatalogSource source = WalkaCatalogSource.cache}) {
    final Map<String, dynamic> rawConfig = _requiredMap(json, 'config');
    final List<dynamic> rawProducts = _requiredList(json, 'products');
    final DateTime? parsedFetchedAt = DateTime.tryParse(_requiredString(json, 'fetched_at'));
    if (parsedFetchedAt == null) throw const FormatException('Catalog fetched_at is invalid.');
    return WalkaCatalogSnapshot(
      config: WalkaStorefrontConfig.fromJson(rawConfig),
      products: rawProducts.map((dynamic value) {
        if (value is! Map) throw const FormatException('Cached products must be objects.');
        return WalkaCatalogProduct.fromJson(Map<String, dynamic>.from(value));
      }).toList(growable: false), source: source, fetchedAt: parsedFetchedAt.toUtc(),
    );
  }
}

abstract final class WalkaCatalogContract {
  static const Set<String> requiredProductIds = <String>{'drawer-organizer', 'stainless-steel-bento-lunch-box'};
  static const Set<String> requiredVariantIds = <String>{'drawer-organizer:white', 'drawer-organizer:gray', 'lunch-box:blue', 'lunch-box:pink', 'lunch-box:green'};
  static const Map<String, Set<String>> allowedVariantIdsByProduct = <String, Set<String>>{
    'drawer-organizer': <String>{'drawer-organizer:white', 'drawer-organizer:gray'},
    'stainless-steel-bento-lunch-box': <String>{'lunch-box:blue', 'lunch-box:pink', 'lunch-box:green'},
  };
  static void validate(WalkaCatalogSnapshot snapshot) {
    if (snapshot.config.brand != 'WALKA') throw const FormatException('Unexpected catalog brand.');
