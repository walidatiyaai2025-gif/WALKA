...
enum WalkaPdpSectionId {
  ...
  static WalkaPdpSectionId parse(Object? value) {
    if (value is! String) {
      throw const FormatException('PDP section id must be a string.');
    }
...
class WalkaPdpLayoutContent {
  ...
  Iterable<WalkaPdpSectionConfig> get visibleSections =>
      sections.where((WalkaPdpSectionConfig section) => section.visible);
  ...
  factory WalkaPdpLayoutContent.fromJson(Map<String, dynamic> json) {
    ...
    if (sectionsValue is! List ||
        sectionsValue.length != WalkaPdpSectionId.values.length) {
      throw const FormatException(
        'PDP layout must contain every supported section exactly once.',
      );
    }
    ...
    for (final Object? raw in sectionsValue) {
      if (raw is! Map) {
        throw const FormatException('PDP layout section must be an object.');
      }
      final Map<String, dynamic> map = Map<String, dynamic>.from(raw);
      final WalkaPdpSectionId id = WalkaPdpSectionId.parse(map['id']);
      if (!seen.add(id)) {
        throw FormatException('Duplicate PDP section: ${id.wireName}');
      }

      final Object? visibleValue = map['visible'];
      if (visibleValue is! bool) {
        throw FormatException(
          'PDP section ${id.wireName} visibility must be boolean.',
        );
      }
      if (requiredVisible.contains(id) && !visibleValue) {
        throw FormatException(
          'Protected PDP section ${id.wireName} cannot be hidden.',
        );
      }
    ...
    if (seen.length != WalkaPdpSectionId.values.length) {
      throw const FormatException('PDP layout is missing supported sections.');
    }

    return WalkaPdpLayoutContent(sections: List.unmodifiable(sections));
  }

  Map<String, dynamic> toJson() => <String, dynamic>{
        'sections': sections
            .map((WalkaPdpSectionConfig section) => section.toJson())
            .toList(growable: false),
      };
}

class WalkaPdpLayoutPayload {
  const WalkaPdpLayoutPayload({
  ...
  factory WalkaPdpLayoutPayload.fromApiJson(Map<String, dynamic> json) {
    ...
    if (dataValue is! Map) {
      throw const FormatException('PDP layout data must be an object.');
    }
    final Map<String, dynamic> data = Map<String, dynamic>.from(dataValue);

    if (data['key'] != 'pdp.layout' || data['type'] != 'pdp.layout') {
      throw const FormatException('Unexpected WALKA PDP layout identity.');
    }
    if (data['schema_version'] != 1) {
      throw const FormatException('Unsupported WALKA PDP layout schema.');
    }

    final Object? revisionValue = data['revision'];
    if (revisionValue is! int || revisionValue < 1) {
      throw const FormatException('PDP layout revision must be positive.');
    }

    final Object? publishedAtValue = data['published_at'];
    if (publishedAtValue is! String || publishedAtValue.trim().isEmpty) {
      throw const FormatException('PDP layout published_at is required.');
    }
    final DateTime? publishedAt = DateTime.tryParse(publishedAtValue);
    if (publishedAt == null) {
      throw const FormatException('PDP layout published_at is invalid.');
    }

    final Object? payloadValue = data['payload'];
    if (payloadValue is! Map) {
      throw const FormatException('PDP layout payload must be an object.');
    }
...
class WalkaPdpLayoutSnapshot {
  ...
  factory WalkaPdpLayoutSnapshot.bundled({DateTime? fetchedAt}) {
  ...
  WalkaPdpLayoutSnapshot asSource(WalkaContentSource nextSource) {
    return WalkaPdpLayoutSnapshot(
      content: content,
      revision: revision,
  ...
  factory WalkaPdpLayoutSnapshot.fromCacheJson(Map<String, dynamic> json) {
    if (json['cache_schema'] != 1) {
      throw const FormatException('Unsupported PDP layout cache schema.');
    }

    final Object? revisionValue = json['revision'];
    if (revisionValue is! int || revisionValue < 1) {
      throw const FormatException('Cached PDP layout revision is invalid.');
    }
    ...
    final DateTime? publishedAt = publishedValue is String
        ...
        : null;
    if (publishedAt == null) {
      throw const FormatException('Cached PDP layout published_at is invalid.');
    }
    ...
    final DateTime? fetchedAt = fetchedValue is String
        ...
        : null;
    if (fetchedAt == null) {
      throw const FormatException('Cached PDP layout fetched_at is invalid.');
    }

    final Object? payloadValue = json['payload'];
    if (payloadValue is! Map) {
      throw const FormatException('Cached PDP layout payload must be an object.');
    }
