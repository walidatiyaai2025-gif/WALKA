...
class WalkaRelatedProductsContent {
  ...
  static const WalkaRelatedProductsContent bundled = WalkaRelatedProductsContent(
    relationships: <WalkaRelatedProductRelationship>[
      ...
      WalkaRelatedProductRelationship(
        ...
        relatedProductIds: <String>['drawer-organizer'],
      ),
    ],
  );
  ...
  factory WalkaRelatedProductsContent.fromJson(Map<String, dynamic> json) {
    ...
    if (relationshipsValue is! List) {
      throw const FormatException(
        'Related-product relationships must be an ordered list.',
      );
    }
    ...
    for (final Object? rawRelationship in relationshipsValue) {
      ...
      final Object? productValue = relationship['product_id'];
      if (productValue is! String || !allowedProductIds.contains(productValue)) {
        throw const FormatException(
          'Related products contain an unsupported stable product ID.',
        );
      }
      if (!seenSources.add(productValue)) {
        throw FormatException('Duplicate related-product source: $productValue');
      }
      if (previousSource != null && productValue.compareTo(previousSource) < 0) {
        throw const FormatException(
          'Related-product source ordering must be deterministic.',
        );
      }
      previousSource = productValue;
      ...
      for (final Object? rawRelatedId in relatedValue) {
        if (rawRelatedId is! String ||
            !allowedProductIds.contains(rawRelatedId)) {
          throw const FormatException(
            'Related products contain an unsupported target product ID.',
          );
        }
        if (rawRelatedId == productValue) {
          throw const FormatException('A product cannot recommend itself.');
        }
        if (!seenRelated.add(rawRelatedId)) {
          throw const FormatException(
            'Related-product targets must be unique per source product.',
          );
        }
        relatedIds.add(rawRelatedId);
      }
...
class WalkaRelatedProductsPayload {
  ...
  factory WalkaRelatedProductsPayload.fromApiJson(Map<String, dynamic> json) {
    ...
    if (publishedAtValue is! String || publishedAtValue.trim().isEmpty) {
      throw const FormatException('Related-products published_at is required.');
    }
...
class WalkaRelatedProductsSnapshot {
  ...
  factory WalkaRelatedProductsSnapshot.fromCacheJson(
    ...
    if (json['cache_schema'] != 1) {
      throw const FormatException(
        'Unsupported related-products cache schema.',
      );
    }
    ...
    if (revisionValue is! int || revisionValue < 1) {
      throw const FormatException(
        'Cached related-products revision is invalid.',
      );
    }
    ...
    if (fetchedAt == null) {
      throw const FormatException(
        'Cached related-products fetched_at is invalid.',
      );
    }
