...
void main() {
  ...
  test('repository provenance contains exactly five released variants in stable order', () async {
    ...
    expect(inspection.valid, isTrue, reason: inspection.diagnostics.map((d) => d.message).join('\n'));
    expect(
      inspection.rows.map((PimgProvenanceRow row) => row.variantId).toList(),
      pimgReleasedVariantIds,
    );
    expect(inspection.admittedCount, 0);
    expect(inspection.pendingCount, 4);
  ...
  test('deterministic report exposes lifecycle counts and every mandatory QA state', () async {
    final PimgProvenanceInspection inspection = await reader.inspect(
      provenancePath: '../docs/ui/PRODUCTION_ASSET_PROVENANCE.json',
      sourceAdmissionPath: '../docs/ui/PRODUCTION_SOURCE_ADMISSION.json',
      mobileRoot: Directory.current.path,
    );
    final Map<String, dynamic> report = jsonDecode(pimgStableJson(inspection)) as Map<String, dynamic>;
    expect(report['state'], 'READY');
    expect((report['counts'] as Map<String, dynamic>)['pending'], 4);
    ...
    for (final dynamic raw in rows) {
      ...
      for (final String key in pimgMandatoryQaChecks) {
        expect(qa.containsKey(key), isTrue, reason: '${row['variantId']} missing $key');
      }
    }
  });

  test('duplicate variant and canonical path are rejected', () async {
    final Directory temp = await Directory.systemTemp.createTemp('walka-pimg-duplicate-');
    ...
    final Map<String, dynamic> second = Map<String, dynamic>.from(rows[1] as Map<String, dynamic>);
  ...
  test('source admission mismatch is rejected instead of silently drifting', () async {
    final Directory temp = await Directory.systemTemp.createTemp('walka-pimg-source-');
    addTearDown(() => temp.delete(recursive: true));
    final Map<String, dynamic> provenance = jsonDecode(
      await File('../docs/ui/PRODUCTION_ASSET_PROVENANCE.json').readAsString(),
    ) as Map<String, dynamic>;
    final List<dynamic> rows = provenance['variants'] as List<dynamic>;
    final Map<String, dynamic> first = Map<String, dynamic>.from(rows.first as Map<String, dynamic>);
  ...
  test('ADMITTED requires fingerprint metadata, approved source and all QA PASS', () async {
    final Directory temp = await Directory.systemTemp.createTemp('walka-pimg-admitted-');
    addTearDown(() => temp.delete(recursive: true));
    final Map<String, dynamic> provenance = jsonDecode(
      await File('../docs/ui/PRODUCTION_ASSET_PROVENANCE.json').readAsString(),
    ) as Map<String, dynamic>;
  ...
  test('protected Images source path is rejected for runtime provenance', () async {
    final Directory temp = await Directory.systemTemp.createTemp('walka-pimg-protected-');
    addTearDown(() => temp.delete(recursive: true));
    final Map<String, dynamic> provenance = jsonDecode(
      await File('../docs/ui/PRODUCTION_ASSET_PROVENANCE.json').readAsString(),
    ) as Map<String, dynamic>;
