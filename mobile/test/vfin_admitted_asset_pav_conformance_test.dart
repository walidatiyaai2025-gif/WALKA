...
void main() {
  ...
  test('every ADMITTED canonical PNG conforms to PAV mechanical media rules', () {
    for (final Map<String, dynamic> row in _rows()) {
      ...
      final dynamic inspection = inspector.inspect(file.readAsBytesSync());
  ...
  test('provenance PAV dimensions stay synchronized with admitted binaries', () {
    for (final Map<String, dynamic> row in _rows()) {
      if (row['lifecycleState'] != 'ADMITTED') continue;
