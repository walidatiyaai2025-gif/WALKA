...
void main() {
  ...
  testWidgets('loading banner remains static under reduced motion',
    ...
    expect(find.byKey(const ValueKey<String>('walka-catalog-status-loading')), findsOneWidget);
  ...
  testWidgets('V130 state surface never renders catalog child while unavailable',
      (WidgetTester tester) async {
    final WalkaCatalogController controller = WalkaCatalogController(
      initialSnapshot: WalkaBundledCatalog.snapshot(),
    );
    addTearDown(controller.dispose);
    await tester.pumpWidget(_app(
      controller: controller,
      child: const WalkaCatalogStateSurfaceV130(child: _CatalogProbe()),
    ));
    expect(find.byKey(const ValueKey<String>('walka-catalog-status-unavailable')), findsOneWidget);
    expect(find.byKey(const ValueKey<String>('walka-catalog-unavailable')), findsOneWidget);
  ...
  testWidgets('public shell stays usable when catalog is unavailable',
    ...
    final WalkaFavoritesController favorites = WalkaFavoritesController(_MemoryFavoritesStore());
...
WalkaCatalogSnapshot _dynamicSnapshot() => WalkaCatalogSnapshot(
...
class _CatalogProbe extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Text(
    'source:${WalkaCatalogScope.of(context).snapshot.source.name}',
  );
}
