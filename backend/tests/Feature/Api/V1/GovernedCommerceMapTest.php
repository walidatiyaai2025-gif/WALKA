...
final class GovernedCommerceMapTest extends TestCase
{
    ...
    {
        parent::setUp(); $this->seed(WalkaCatalogSeeder::class); $this->actor = hash('sha256', 'cms-060-governed-commerce-test');
    }
    ...
    public function test_authoring_normalizes_and_publishes_only_canonical_amazon_destination(): void
    {
        ...
        $entry = $service->saveDraft(CommerceMapContentDefinition::KEY, CommerceMapContentDefinition::TYPE, $this->payload(), 0, $this->actor);
        ...
        $response = $this->getJson('/api/v1/commerce/amazon/lunch-box:blue?market=US')->assertOk()
            ->assertJsonPath('data.variant_id', 'lunch-box:blue')->assertJsonPath('data.asin', 'B0FQN4L8MW')
            ->assertJsonPath('data.destination_url', 'https://www.amazon.com/dp/B0FQN4L8MW')->assertJsonPath('meta.market_source', 'query')->assertJsonPath('meta.retry_safe', true);
    ...
    public function test_missing_market_has_deterministic_us_fallback_and_retry_identity(): void
    {
        ...
        $first->assertJsonPath('data.region_market', 'US')->assertJsonPath('meta.market_source', 'default_us');
    ...
    {
        $this->publishPayload($this->payload());
        $malformed = $this->getJson('/api/v1/commerce/amazon/lunch-box:blue?market=evil')->assertStatus(422)->assertJsonPath('error.code', 'amazon_destination_rejected');
        $this->assertStringNotContainsString('destination_url', $malformed->getContent());
        $unmapped = $this->getJson('/api/v1/commerce/amazon/lunch-box:blue?market=CA')->assertNotFound()->assertJsonPath('error.code', 'amazon_destination_unmapped');
    ...
    {
        $this->publishPayload($this->payload()); ProductVariant::query()->whereKey('lunch-box:blue')->increment('revision');
        $response = $this->getJson('/api/v1/commerce/amazon/lunch-box:blue?market=US')->assertStatus(503)->assertJsonPath('error.code', 'commerce_map_invalid');
    ...
    {
        $service = app(ContentRevisionService::class);
        $service->saveDraft(CommerceMapContentDefinition::KEY, CommerceMapContentDefinition::TYPE, $this->payload(), 0, $this->actor);
        ProductVariant::query()->whereKey('lunch-box:blue')->increment('revision');
        try {
            $service->restoreDraftFromRevision(CommerceMapContentDefinition::KEY, 1, 1, $this->actor, 'rollback stale mapping test');
            $this->fail('Stale historical CommerceMap revision should not restore.');
        } catch (ValidationException) { $this->assertTrue(true); }
        $service->saveDraft(CommerceMapContentDefinition::KEY, CommerceMapContentDefinition::TYPE, $this->payload(2), 1, $this->actor);
        $publishAt = CarbonImmutable::parse('2026-08-16T15:00:00Z');
        app(ContentScheduleService::class)->schedule(CommerceMapContentDefinition::KEY, 2, $publishAt, null, $this->actor);
        ProductVariant::query()->whereKey('lunch-box:blue')->increment('revision');
    ...
    {
        return ['mappings' => [[
            'variant_id' => 'lunch-box:blue', 'variant_revision' => $variantRevision, 'region_market' => 'us', 'asin' => 'b0fqn4l8mw',
            'destination_url' => 'https://example.invalid/open-redirect-is-ignored', 'cta_key' => 'buy_on_amazon',
            'disclosure_key' => 'amazon_purchase_disclosure', 'entitlements' => ['amazon_purchase'], 'active' => true,
            'trace' => ['source' => 'cms', 'reference' => 'CMS-060'],
        ]]];
    }
    ...
    {
        $service = app(ContentRevisionService::class);
        $service->saveDraft(CommerceMapContentDefinition::KEY, CommerceMapContentDefinition::TYPE, $payload, 0, $this->actor);
        $service->publish(CommerceMapContentDefinition::KEY, 1, $this->actor);
    }
}
