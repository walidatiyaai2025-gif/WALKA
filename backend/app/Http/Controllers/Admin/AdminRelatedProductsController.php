...
final class AdminRelatedProductsController extends Controller
{
    ...
    public function edit(Request $request): View
    {
        ...
        abort_unless(
            $entry->content_type === RelatedProductsContentDefinition::TYPE,
            409,
            'The reserved pdp.related_products content key has an incompatible content type.',
        );
    ...
    public function update(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()
                ...
                ->withErrors([
                    'revision' => 'Related products changed in another session. Current values were reloaded; review before saving again.',
                ]);
        }

        return redirect()
            ->route('admin.content.pdp.related-products.edit')
            ->with('status', 'Related-product draft saved. Live Product Detail pages remain unchanged until Publish is used.');
    }
    ...
    public function publish(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()
                ->route('admin.content.pdp.related-products.edit')
                ->withErrors([
                    'revision' => 'Publish blocked because related products changed in another session. Review the latest draft first.',
                ]);
        }

        return redirect()
            ->route('admin.content.pdp.related-products.edit')
            ->with('status', 'Related products published. Compatible PDPs can apply the approved stable-ID recommendations without a new app build.');
    }
    ...
    public function restore(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()
                ->route('admin.content.pdp.related-products.edit')
                ->withErrors([
                    'revision' => 'Restore blocked because related products changed in another session. Reload and review the latest revision.',
                ]);
        }

        return redirect()
            ->route('admin.content.pdp.related-products.edit')
            ->with('status', 'Historical related products restored into a new private draft. Review before publishing.');
    }
    ...
    private function payloadFromRequest(Request $request): array
    {
        ...
        foreach (array_keys($rawRelated) as $sourceId) {
            if (! is_string($sourceId) || ! isset($known[$sourceId])) {
                throw ValidationException::withMessages([
                    'related' => ['Unknown source product ID was submitted.'],
                ]);
            }
            ...
            foreach (array_keys($rawRelated[$sourceId]) as $candidateId) {
                if (! is_string($candidateId) || ! isset($known[$candidateId]) || $candidateId === $sourceId) {
                    throw ValidationException::withMessages([
                        "related.$sourceId" => ['Unknown or self-referencing related product was submitted.'],
                    ]);
                }
            }
        }
